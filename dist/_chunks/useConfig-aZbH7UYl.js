...
const instance = axios.create({
  baseURL: `${process.env.STRAPI_ADMIN_BACKEND_URL}/${index.pluginId}`,
  headers: {
...
function useConfig(newConfig) {
  const [config, setConfig] = React.useState();
  const onResponse = ({ data }) => setConfig(data.data);
  const onError = (error) => {
    console.error(error);
    setConfig(null);
  };
  React.useEffect(() => {
    instance.get(endpoint).then(onResponse).catch(onError);
  }, []);
  React.useEffect(() => {
    if (newConfig) {
      setConfig(void 0);
      instance.put(endpoint, { data: newConfig }).then(onResponse).catch(onError);
    }
  }, [newConfig]);
  return config;
}
exports.useConfig = useConfig;
//# sourceMappingURL=useConfig-aZbH7UYl.js.map
